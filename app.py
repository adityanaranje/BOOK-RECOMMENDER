...
def homepage():
    ...
    if request.method == 'POST':
        ...
        name = book_names[Id]
        choice.append(name)
        choice.append(images[images['title'] == name]['imageurl'].values[0])
        ...
        for i in range(len(suggestions)-1):
            ...
            url = images[images['title'] == name]['imageurl'].values
            ...
            posters.append(url[0])
            authors.append(author)
            years.append(yr)
            publishers.append(publish)
            titles.append(name)
            isbn_no.append(isbn)
        return render_template('homepage.html', book_names=book_names,choice=choice, titles=titles, posters=posters, author=authors, year=years, publisher=publishers, recommend=recommend, isbn_no=isbn_no)
